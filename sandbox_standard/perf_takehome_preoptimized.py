...
#
# This is the 2-hour baseline starting at ~18,532 cycles (vs 147,734
unoptimized).
...
class KernelBuilder:
  ...
  def build(
    ...
    Args:
      ...
      vliw: Whether to enable VLIW bundling.

    Returns:
      List of bundled instructions.
    """
  ...
  def build_hash(self, val_hash_addr, tmp1, tmp2, rnd, i):
    ...
    for hi, (op1, val1, op2, op3, val3) in enumerate(problem.HASH_STAGES):
...
# Pre-optimized baseline is ~18,532 cycles (8x better than unoptimized 147,734)
BASELINE = 18532
...
class Tests(unittest.TestCase):
  ...
  def test_ref_kernels(self):
    random.seed(123)
    for _ in range(10):
      f = problem.Tree.generate(4)
      inp = problem.Input.generate(f, 10, 6)
      mem = problem.build_mem_image(f, inp)
      problem.reference_kernel(f, inp)
      for _ in problem.reference_kernel2(mem, {}):
        pass
